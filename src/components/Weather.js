...
const Weather = (props) => {
    ...
    if (props.icon === 'ic13d') 
        icon = ic13d
    if (props.icon === 'ic050d') 
        icon = ic50d

    return (
        <div>

            {props.city && <p className="city">
                {props.city}</p>}

            {props.city && <div className="weather-icon__container">
                <img src={icon} alt="weather icon"/>
            </div>}
            {props.description && <p>{props
                    .description
                    .toUpperCase()}</p>}

            {props.temperature && <div className="temperature__container">
                <img className="temperature-icon" src={temperature} alt="temperature"/>
                <p className="temperature">: {props.temperature}
                </p>
